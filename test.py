...
def evaluate_r(cfgs, refine_in):
    ...
    refine_ckpt = os.path.join(
        out_dir,
        "refine",
        "refine_dual.pth" if len(refine_in) == 2 else "refine_intra.pth",
    )
    if len(refine_in) == 2:
        checkpoint_label = "refine_dual"
    else:
        checkpoint_label = "refine_intra"
    try:
        refine_net.load_state_dict(
            torch.load(refine_ckpt, map_location=torch.device('cuda:{}'.format(gpu)))
        )
    except RuntimeError as exc:
        raise RuntimeError(
            "Failed to load '{}' with band_mode='{}' (expected {} input channels). "
            "Please retrain mode r before running eval_r.".format(
                checkpoint_label,
    ...
    refine_net.eval()
    print(
        "=> eval_r preprocess: band_mode={} in_channels={} score_topk_ratio={:.2%}".format(
            refine_runtime_cfg["band_mode"],
            refine_in_channels,
    ...
    result_str = "{} - AUC:{:.3f}  AP:{:.3f}".format(histogram_name, auc, ap)
    print(result_str)
    result_payload = {
        "method": histogram_name,
        "auc": float(auc),
        "ap": float(ap),
    }
    with open(os.path.join(out_dir, "eval_r_results.txt"), "w") as f:
        f.write(result_str + "\n")
    with open(os.path.join(out_dir, "eval_r_results.json"), "w") as f:
